...
bool init(){
    ...
    glBindBuffer(GL_ARRAY_BUFFER,positionVBO);
    glVertexAttribPointer(POSITION_IND,3,GL_FLOAT,GL_FALSE,3* sizeof(GLfloat),(const void *)NULL);
    glEnableVertexAttribArray(POSITION_IND);

    glBindBuffer(GL_ARRAY_BUFFER,mvpVBO);
    glVertexAttribPointer(MVP_IND+0,4,GL_FLOAT,GL_FALSE, sizeof(Matrix),(const void *) NULL);
    glVertexAttribPointer(MVP_IND+1,4,GL_FLOAT,GL_FALSE, sizeof(Matrix),(const void *) (sizeof(GL_FLOAT)*4));
    glVertexAttribPointer(MVP_IND+2,4,GL_FLOAT,GL_FALSE, sizeof(Matrix),(const void *) (sizeof(GL_FLOAT)*8));
    glVertexAttribPointer(MVP_IND+3,4,GL_FLOAT,GL_FALSE, sizeof(Matrix),(const void *) (sizeof(GL_FLOAT)*12));
...
void update(float deltaTime){
    ...
    frustumH = tanf(float(60.0f / 360.0f * M_PI)) * 1.5f;
    frustumW = frustumH * aspect;
    frustum(&perspective,frustumW,frustumH,1.0f,20.0f);
    glBindBuffer(GL_ARRAY_BUFFER, mvpVBO);
    matrixBuf = (Matrix *) glMapBufferRange(GL_ARRAY_BUFFER, 0, sizeof(Matrix), GL_MAP_WRITE_BIT);
    Matrix modelview;
    ...
    rotate(&modelview, angle, 1.0, 0.0, 1.0f);
...
Java_com_szj_testjni_cube_CubeLib_init(JNIEnv *env, jclass type) {
...
Java_com_szj_testjni_cube_CubeLib_resize(JNIEnv *env, jclass type, jint width, jint height) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_szj_testjni_cube_CubeLib_step(JNIEnv *env, jclass type) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_szj_testjni_cube_CubeLib_destory(JNIEnv *env, jclass type) {
